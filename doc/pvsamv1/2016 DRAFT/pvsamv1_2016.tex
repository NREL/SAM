...
Snow coverage is a fraction of the height of a subarray row as shown in Figure \ref{fig-snowCoverage} and determined by the number of modules along side of row input (\texttt{nmody} in SSC). The snow model only considers sliding. It does not consider other snow removal processes such as snow melting or removal by wind considered.
...
\textbf{1. Determine amount of snow coverage based on snow depth}

Set snow coverage to one $C_{snow} = 1$ if all of the following conditions are met:
\begin{itemize}
...
If $D_{snow} < 1$ cm, then set $C_{snow}=0$. This ensures that the snow eventually melts off of a subarray with a tilt angle of zero $\beta_s=0$. This adaptation is specific to SAM and not part of the original Marion model, which is limited to $10^\circ \text{C} < \beta_{S} < 45^\circ \text{C}$.

\texbf{2. Decrease amount of snow coverage if sliding occurs}

Because SAM does not calculate surface angles for night-time time steps ($sunup = 0$), the snow model sets the night-time subarray tilt angle to the base value $\beta_0$ to ensure it accounts for snow sliding that may occur at night. Note that this value is not reflected in the $\beta_S$ calculated by the photovoltaic model. 

If the ambient temperature for the current time step is high enough to initiate snow sliding in the current hour, $T_{dry}> \frac{G_{b}+G_{d}}{m}$, then reduce snow coverage in the current time step $C_{snow}$ by 10\%. Otherwise, set $C_{snow}$ to its value in the previous time step. For subhourly data, the snow cover is reduced by the time step's fraction of an hour to ensure that the reduction over the hour is 10\% of the previous hours snow coverage value. For example, for 15-minute time steps, the reduction would be $0.10 \times 15/60$.

\begin{equation}
	s.s.a. = 1.97 * sin( \beta_{S} )
	\label{eqn-snowSlideAmount}
\end{equation}

The relationship $T_{dry}> \frac{G_{b}+G_{d}}{m}$ was determined experimentally with the constant parameter $m=-80\ W/m^2/^\circ C$) as described in \citet{marion-snowmodel}.
