...
Usage of the snow model amounts to a check box labeled 'Estimate losses due to snow' which can be found at the bottom of the 'Shading and Snow' input page. If this is check box is activated, then the snow coverage losses will be automatically calculated as described in this section. To note, not all weatherfiles will contain snow depth data or else may contain a few 'bad' snow depth values (defined as any value that is less than zero, greater than 20 feet, or non-existent). If a bad snow depth value is found during the simulation then that value will be replaced with zero depth. This replacement will only occur for the first 500 bad values, after which if another bad snow depth value is encountered then the simulation will fail.
