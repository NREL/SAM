...
****SOLAR IRRADIANCE DATA CAN BE DNI/DHI, DNI/GHI, GHI/DHI, OR POA. DNI/DHI IS DEFAULT, WHEN OTHER OPTIONS ARE USED, SAM CALCULATES MISSING COMPONENT FROM OTHER TWO. POA BYPASSES CALCULATIONS EXCEPT FOR SHADING REF BYPASSING POA SECTION****
...
****SSC OPTION TO INPUT DATA ARRAY INSTEAD OF WEATHER FILE****
...
SAM's photovoltaic model reads and stores the weather file data shown in Table~\ref{tab-wfdata}. It ignores any additional data elements that may be included in the different weather file formats. Some additional data elements, such as relative humidity, ****RELATIVE HUMIDITY REQUIRED FOR HEAT TRANSFER CELL TEMP MODEL**** are required for SAM's concentrating solar power performance models, but not for the photovoltaic model. SAM also ignores supporting data, such as the source and uncertainty flags in the TMY3 file format.
...
$E_g$ & Global horizontal irradiance (W/m$^2$) ****NOT REQUIRED & total radiation on a horizontal surface \\
...
POA & ****POA IS OPTIONAL & ****PLANE OF ARRAY\\
...
\multicolumn{3}{l}*Albedo may be from weather file or set to default value of 0.2 if weather file does not contain albedo data.\\
\end{tabular}
