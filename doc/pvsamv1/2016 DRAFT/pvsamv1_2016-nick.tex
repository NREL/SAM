...
Lead-acid or lithium-ion chemistry & \texttt{batt\_type} \\
Lead-acid type & \texttt{batt\_lead\_type} \\
Lithium-ion type & \texttt{batt\_lithium\_type} \\
\midrule
...
\textbf{Average discharge time (hr)} & \texttt{batt\_time\_capacity} \\
\midrule
...
\textbf{Target grid power (kW)} & \texttt{batt\_target\_power}\\
...
SAM's manual dispatch controller is described in detail in \citep{diorio2015a}.  Several additional modes have since been added to provide more capabilities to the user.  Regardless of which dispatch model is selected, the minimum and maximum states of charge as well as the minimum time at charge state must be specified.  There are currently four available dispatch models, including the manual dispatch model.
...
There is also an automated grid power target model, which allows the user to input the maximum grid power limit for every timestep.  The automated controller uses a 24-hour look ahead to predict the grid power required.  The battery is then dispatched to reduce all grid power events which exceed the target threshold for that timestep.  For all timesteps where the grid power is less than the threshold, the battery is allowed to charge from the grid until the power is equal to the threshold.  
