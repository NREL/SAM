...
bool CodeGen_php::Input(ssc_data_t p_data, const char *name, const wxString &folder, const int &array_matrix_threshold)
{
	...
	case SSC_ARRAY:
		...
		else
		{
			...
			if (dbl_value > 1e38) dbl_value = 1e38;
			fprintf(m_fp, " %.17g ];\n", dbl_value);
			fprintf(m_fp, "	sscphp_data_set_array( $dat, \"%s\", $%s, %d);\n", name, name, len);
		}
		break;
	case SSC_MATRIX:
		p = ::ssc_data_get_matrix(p_data, name, &nr, &nc);
		...
		else
		{
			...
			if (dbl_value > 1e38) dbl_value = 1e38;
			fprintf(m_fp, " %.17g ]];\n", dbl_value);
			fprintf(m_fp, "	sscphp_data_set_matrix( $dat, \"%s\", $%s, %d, %d );\n", name, name, nr, nc);
		}
		break;
		// TODO tables in future
	}
...
bool CodeGen_php::SupportingFiles()
{
	...
	fprintf(f, "	zend_resource *res;\n");
	fprintf(f, "	spobj *p;\n");
	fprintf(f, "	if ( zend_parse_parameters( ZEND_NUM_ARGS() TSRMLS_CC, \"r\", &res) == FAILURE )\n");
	fprintf(f, "		RETURN_FALSE;\n");
	fprintf(f, "	if ((p = (spobj *)zend_fetch_resource2_ex(res, -1, spobj_name, spobj_id )) == NULL) {\n");
	fprintf(f, "	    RETURN_FALSE;\n");
	fprintf(f, "	}\n");
	...
	fprintf(f, "  	if ( (data =zend_hash_get_current_data_ex(hash, &pointer1 )) != NULL\n");
	fprintf(f, "	 || Z_TYPE_P(data) != IS_ARRAY )\n");
	fprintf(f, "	 	return;\n");
	...
	fprintf(f, "			(data = zend_hash_get_current_data_ex(hash, &pointer2 )) != NULL;\n");
	...
	fprintf(f, "	}\n");
	fprintf(f, "	if (!p || !p->p_dat) RETURN_FALSE;\n");
	fprintf(f, "	if ((t = (spobj *)zend_fetch_resource2_ex(res, -1, spobj_name, spobj_id )) == NULL) {\n");
	fprintf(f, "	    RETURN_FALSE;\n");
	fprintf(f, "	}\n");
	...
	fprintf(f, "	zend_resource *res1, *res2;\n");
	fprintf(f, "	if ( zend_parse_parameters( ZEND_NUM_ARGS() TSRMLS_CC, \"rr\", &res1, &res2 ) == FAILURE )\n");
	fprintf(f, "		RETURN_FALSE;\n");
	fprintf(f, "	if ((mm = (spobj *)zend_fetch_resource2_ex(res1, -1, spobj_name, spobj_id )) == NULL) {\n");
	fprintf(f, "	    RETURN_FALSE;\n");
	fprintf(f, "	}\n");
	fprintf(f, "	if ( !mm || !mm->p_mod ) RETURN_FALSE;\n");
	fprintf(f, "	if ((dd = (spobj *)zend_fetch_resource2_ex(res2, -1, spobj_name, spobj_id )) == NULL) {\n");
	fprintf(f, "	    RETURN_FALSE;\n");
	fprintf(f, "	}\n");
