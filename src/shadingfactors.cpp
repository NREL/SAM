...
enum { IDSF_GRID, IDSF_SHADINGVAL, IDSF_APPLY, IDSF_IMPORT, IDSF_EXPORT };

BEGIN_EVENT_TABLE(ShadingCtrl, wxPanel)

	EVT_GRID_CMD_CELL_CHANGE( IDSF_GRID, ShadingCtrl::OnGridCellChange)
	EVT_GRID_CMD_SELECT_CELL( IDSF_GRID, ShadingCtrl::OnGridCellSelect)
	EVT_GRID_CMD_RANGE_SELECT( IDSF_GRID, ShadingCtrl::OnGridRangeSelect)
	EVT_GRID_CMD_EDITOR_HIDDEN( IDSF_GRID, ShadingCtrl::OnGridEditorHidden)
	EVT_GRID_CMD_EDITOR_SHOWN( IDSF_GRID, ShadingCtrl::OnGridEditorShown)

	EVT_BUTTON( IDSF_IMPORT, ShadingCtrl::OnImport)
	EVT_BUTTON( IDSF_EXPORT, ShadingCtrl::OnExport)

	EVT_BUTTON( IDSF_APPLY, ShadingCtrl::OnApply )
	EVT_NUMERIC( IDSF_SHADINGVAL, ShadingCtrl::OnApply )

END_EVENT_TABLE()

DEFINE_EVENT_TYPE( wxEVT_SHADINGCTRL_CHANGE )
...
ShadingCtrl::ShadingCtrl(wxWindow *parent, int id, const wxPoint &pos, const wxSize &sz)
...
ShadingCtrl::~ShadingCtrl()
{
	/* nothing to do */
}

void ShadingCtrl::SetTitle(  wxString &title)
{
	m_title->SetLabel( title );
}

wxString ShadingCtrl::GetTitle()
{
	return m_title->GetLabel();
}

void ShadingCtrl::SetLegend(  wxString &legend)
{
	m_legend->SetLabel( legend );
}

wxString ShadingCtrl::GetLegend()
{
	return m_legend->GetLabel();
}


void ShadingCtrl::SetData(const matrix_t<float> &data)
{
...
matrix_t<float> ShadingCtrl::GetData()
{
	return mData;
}

void ShadingCtrl::UpdateGrid()
{
...
void ShadingCtrl::UpdateCell(int r, int c)
{
...
void ShadingCtrl::OnGridCellChange(wxGridEvent &evt)
{
	int r = evt.GetRow();
	int c = evt.GetCol();
...
void ShadingCtrl::ApplyVal(int r, int c, double sf)
{
	if (sf<0.0) sf = 0.0;
	if (sf>1.0) sf = 1.0;
...
void ShadingCtrl::OnGridRangeSelect(wxGridRangeSelectEvent &evt)
{
	if (evt.CmdDown())
	{
		evt.Veto();
...
void ShadingCtrl::OnGridEditorShown(wxGridEvent &evt)
{
	bSkipSelect = true;
}

void ShadingCtrl::OnGridEditorHidden(wxGridEvent &evt)
{
	bSkipSelect = false;
}

void ShadingCtrl::OnGridCellSelect(wxGridEvent &evt)
{
	if (evt.CmdDown())
	{
		evt.Veto();
...
void ShadingCtrl::OnApply(wxCommandEvent &evt)
{
...
void ShadingCtrl::OnImport(wxCommandEvent &evt)
{
...
void ShadingCtrl::OnExport(wxCommandEvent &evt)
{
...
void ShadingCtrl::DispatchEvent()
{	
	wxCommandEvent change(wxEVT_SHADINGCTRL_CHANGE, this->GetId() );
...
public:
			...
			wxDEFAULT_DIALOG_STYLE|wxRESIZE_BORDER)
	{
		...
		m_mxh = new ShadingCtrl( m_scrollWin, wxID_ANY );
