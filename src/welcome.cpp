...
enum {
    ID_CREATE_PROJECT = wxID_HIGHEST + 556, ID_OPEN_EXISTING, ID_RECENT_FILES,
...
WelcomeScreen::WelcomeScreen(wxWindow *parent)
    ...
    m_htmlWin->SetPage(
            "<html><body><font color=#a9a9a9 face=\"Segoe UI Light\" size=10>Loading news...</font></body></html>");
    ...
    m_btnGetStarted = new wxMetroButton(this, ID_QUICK_START, "Quick start for new users", wxNullBitmap,
    ...
    if (!vm.initialize(&env) || !vm.run()) {
        if (vm.error().Lower() == "[0] no bytecode loaded") {
            if (strlen(sam_api_key) == 0)
                UpdateMessagesHtml(
                        "<html><body><font color=#a9a9a9 face=\"Segoe UI Light\" size=10>Please setup API keys, see private.h for details...</font></body></html>");
            else
