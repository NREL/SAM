...
static void dump_variable(FILE *fp, ssc_data_t p_data, const char *name) { // .17g to .17g for full double precesion.
    ssc_number_t value;
    ssc_number_t *p;
    int len, nr, nc;
    wxString str_value;
            ...
            if (var_type == SSC_INPUT || var_type == SSC_INOUT) {
                ...
                if (existing_type != data_type) {
                    if (VarValue *vv = GetInput(name)) {
                        if (!field.IsEmpty()) {
                            if (vv->Type() != VV_TABLE)
                                ih.Error("SSC variable has table:field specification, but '" + name +
                            ...
                            if (do_copy_var) {
                                if (VarValue *vv_field = vv->Table().Get(field)) {
                                    if (!VarValueToSSC(vv_field, p_data, name + ":" + field))
                                        ih.Error("Error translating table:field variable from SAM UI to SSC for '" +
                                                 name + "':" + field);
                                }
                            }
                        ...
                        ih.Error("SSC requires input '" + name +
