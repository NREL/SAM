...
public:
	...
	void SetVerifyHttps( bool verify ) { m_verifyHttps = verify; }
	void SetTimeout( int seconds ) { m_timeoutSec = seconds; }
	void SetThread( wxThread *thread ) { m_thread = thread; }
	wxThread *GetThread() { return m_thread; }
	...
	CURL *GetCurlPointer() { return m_pCURL; }
...
protected:
	bool m_verifyHttps;
	int m_timeoutSec;
	wxThread *m_thread; // non-null if this was created within a thread
