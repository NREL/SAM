...
//cpg This file goes in the html folder. The help compiler generates a new version
//cpg of this file each time html is generated. Replace that file with this one
//cpg for simple search
...
if (isNaN(per_page)) per_page = 100; //cpg set default results per page to 100
...
if (isNaN(andq))
{
	//cpg set search mode to any
	//cpg if (typeof(DefaultToAnd) != "undefined" && DefaultToAnd == 1)
	//cpg 	andq = 1;
	//cpg else
		andq = 0;
}
...
function ZoomShowResults()
{
	...
	//cpg Only display link to topic, not description from topic title
	...
	//	}
		...
		if (DisplayTerms == 1)
  //cpg don't display score or terms matched
  //cpg		info_str += STR_RESULT_TERMS_MATCHED + " " + output[arrayline][2];
  //cpg
	//cpg	if (DisplayScore == 1) {
	//cpg		if (info_str.length > 0)
	//cpg			info_str += "&nbsp; - &nbsp;";
	//cpg		info_str += STR_RESULT_SCORE + " " + score;
	//cpg	}
...
function ZoomShowSummary()
{
	...
	if (matches == 0)
		...
		//cpg don't show number of returned results document.writeln(PrintNumResults(fullmatches) + " " + STR_SUMMARY_FOUND_CONTAINING_ALL_TERMS + " ");
...
//cpg	ZoomShowHeading();
ZoomShowSummary();
//cpg	ZoomShowCatSummary();
//cpg	ZoomShowPagesCount();
//cpg	ZoomShowRecommended();
//cpg	ZoomShowSorting();
	ZoomShowResults();

//cpg	ZoomShowPageNumbers();

//cpg	ZoomShowSearchTime();
}
