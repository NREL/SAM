...
if __name__ == '__main__':
    ...
    T_amb_des_levels = [30, 35, 40, 45]
    dT_mc_approach_levels = [2, 4, 6, 8, 10]
    fan_frac_power_levels = [0.01, 0.02, 0.04, 0.06]
    
    for i_fan_frac_power in fan_frac_power_levels:

        for i_dT_mc_approach in dT_mc_approach_levels:
        
            for i_T_amb_des in T_amb_des_levels:
                ...
                # Then modify design parameters to current parametric run values
                
                sco2_des_par_in["T_amb_des"] = i_T_amb_des
                sco2_des_par_in["dT_mc_approach"] = i_dT_mc_approach
                sco2_des_par_in["fan_power_frac"] = i_fan_frac_power
