...
std::string extractor_interpreter::spell_list(lk::list_t *l, std::vector<std::string> *vertex_names,
...
std::string extractor_interpreter::spell_out(lk::expr_t *n, std::vector<std::string> *vertex_names,
                                             std::vector<bool> *vertex_is_ssc) {
    digraph *graph = SAM_config_to_variable_graph.find(active_config)->second;
    std::string s;
    ...
    if (list_t *n1 = dynamic_cast<list_t *>(root)) {
            ...
            switch (n4->oper) {
                ...
                case expr_t::TYPEOF:
                    if (lk::iden_t *iden = dynamic_cast<lk::iden_t *>(n4->left)) {
                        ...
                        std::cout << "error at line: " << n4->line()
                                  << " argument to typeof(...) must be an identifier\n";
                        return false;
                    }
                    break;
                case expr_t::INITVEC: {
        ...
        if (x) {
            if (n6->constval) {
                ...
                std::cout
                        << " overriding previous non-const identifier with const-ness not allowed: " + n6->name + "\n";
                result.nullify();
//                return false;
            }
...
std::string translator::get_vv_type(const lk_string &name) {
    ...
    std::vector<std::string> vv_typestr = {"undefined", "float", "util::matrix_t<float>", "util::matrix_t<float>",
                                           "std::string", "var_table", "binary"};
