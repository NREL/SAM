...
SAM_EXPORT void SAM_Windpower_AdjustmentFactors_adjust_en_hourly_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
...
SAM_EXPORT void SAM_Windpower_AdjustmentFactors_adjust_hourly_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
...
SAM_EXPORT double SAM_Windpower_AdjustmentFactors_adjust_en_hourly_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "adjust_en_hourly", &result))
		make_access_error("SAM_Windpower", "adjust_en_hourly");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Windpower_AdjustmentFactors_adjust_hourly_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "adjust_hourly", length);
	if (!result)
		make_access_error("SAM_Windpower", "adjust_hourly");
	});
	return result;
}
