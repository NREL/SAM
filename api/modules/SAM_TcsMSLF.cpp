...
SAM_EXPORT int SAM_TcsMSLF_execute(SAM_table data, int verbosity, SAM_error* err){
	return SAM_module_exec("tcsmslf", data, verbosity, err);
}
...
SAM_EXPORT void SAM_TcsMSLF_AdjustmentFactors_adjust_constant_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
...
SAM_EXPORT void SAM_TcsMSLF_AdjustmentFactors_adjust_periods_mset(SAM_table ptr, double* mat, int nrows, int ncols, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_matrix(ptr, "adjust_periods", mat, nrows, ncols);
	});
}

SAM_EXPORT void SAM_TcsMSLF_AdjustmentFactors_adjust_timeindex_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
...
SAM_EXPORT double SAM_TcsMSLF_AdjustmentFactors_adjust_constant_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "adjust_constant", &result))
		make_access_error("SAM_TcsMSLF", "adjust_constant");
	});
	return result;
}
...
SAM_EXPORT double SAM_TcsMSLF_AdjustmentFactors_adjust_en_periods_nget(SAM_table ptr, SAM_error *err){
	...
	if (!ssc_data_get_number(ptr, "adjust_en_periods", &result))
		make_access_error("SAM_TcsMSLF", "adjust_en_periods");
	});
	return result;
}

SAM_EXPORT double SAM_TcsMSLF_AdjustmentFactors_adjust_en_timeindex_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
...
SAM_EXPORT double* SAM_TcsMSLF_AdjustmentFactors_adjust_periods_mget(SAM_table ptr, int* nrows, int* ncols, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_matrix(ptr, "adjust_periods", nrows, ncols);
	if (!result)
		make_access_error("SAM_TcsMSLF", "adjust_periods");
	});
	return result;
}

SAM_EXPORT double* SAM_TcsMSLF_AdjustmentFactors_adjust_timeindex_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "adjust_timeindex", length);
	if (!result)
		make_access_error("SAM_TcsMSLF", "adjust_timeindex");
	});
	return result;
}
...
SAM_EXPORT double* SAM_TcsMSLF_Outputs_annual_energy_distribution_time_mget(SAM_table ptr, int* nrows, int* ncols, SAM_error *err){
	...
	if (!result)
		make_access_error("SAM_TcsMSLF", "annual_energy_distribution_time");
	});
	return result;
}
