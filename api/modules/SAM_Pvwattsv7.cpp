...
SAM_EXPORT void SAM_Pvwattsv7_Shading_shading_azal_mset(SAM_table ptr, double* mat, int nrows, int ncols, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_matrix(ptr, "shading_azal", mat, nrows, ncols);
	});
}

SAM_EXPORT void SAM_Pvwattsv7_Shading_shading_diff_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "shading_diff", number);
	});
}

SAM_EXPORT void SAM_Pvwattsv7_Shading_shading_en_azal_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "shading_en_azal", number);
	});
}

SAM_EXPORT void SAM_Pvwattsv7_Shading_shading_en_diff_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "shading_en_diff", number);
	});
}

SAM_EXPORT void SAM_Pvwattsv7_Shading_shading_en_mxh_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "shading_en_mxh", number);
	});
}

SAM_EXPORT void SAM_Pvwattsv7_Shading_shading_en_string_option_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "shading_en_string_option", number);
	});
}

SAM_EXPORT void SAM_Pvwattsv7_Shading_shading_en_timestep_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "shading_en_timestep", number);
	});
}

SAM_EXPORT void SAM_Pvwattsv7_Shading_shading_mxh_mset(SAM_table ptr, double* mat, int nrows, int ncols, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_matrix(ptr, "shading_mxh", mat, nrows, ncols);
	});
}

SAM_EXPORT void SAM_Pvwattsv7_Shading_shading_string_option_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "shading_string_option", number);
	});
}

SAM_EXPORT void SAM_Pvwattsv7_Shading_shading_timestep_mset(SAM_table ptr, double* mat, int nrows, int ncols, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_matrix(ptr, "shading_timestep", mat, nrows, ncols);
	});
}

SAM_EXPORT void SAM_Pvwattsv7_AdjustmentFactors_adjust_constant_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "adjust_constant", number);
	});
}

SAM_EXPORT void SAM_Pvwattsv7_AdjustmentFactors_adjust_en_hourly_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "adjust_en_hourly", number);
	});
}

SAM_EXPORT void SAM_Pvwattsv7_AdjustmentFactors_adjust_en_periods_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "adjust_en_periods", number);
	});
}

SAM_EXPORT void SAM_Pvwattsv7_AdjustmentFactors_adjust_en_timeindex_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "adjust_en_timeindex", number);
	});
}

SAM_EXPORT void SAM_Pvwattsv7_AdjustmentFactors_adjust_hourly_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "adjust_hourly", arr, length);
	});
}

SAM_EXPORT void SAM_Pvwattsv7_AdjustmentFactors_adjust_periods_mset(SAM_table ptr, double* mat, int nrows, int ncols, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_matrix(ptr, "adjust_periods", mat, nrows, ncols);
	});
}

SAM_EXPORT void SAM_Pvwattsv7_AdjustmentFactors_adjust_timeindex_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
...
SAM_EXPORT double* SAM_Pvwattsv7_Shading_shading_azal_mget(SAM_table ptr, int* nrows, int* ncols, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_matrix(ptr, "shading_azal", nrows, ncols);
	if (!result)
		make_access_error("SAM_Pvwattsv7", "shading_azal");
	});
	return result;
}
...
SAM_EXPORT double SAM_Pvwattsv7_Shading_shading_diff_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "shading_diff", &result))
		make_access_error("SAM_Pvwattsv7", "shading_diff");
	});
	return result;
}
...
SAM_EXPORT double SAM_Pvwattsv7_Shading_shading_en_azal_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "shading_en_azal", &result))
		make_access_error("SAM_Pvwattsv7", "shading_en_azal");
	});
	return result;
}
...
SAM_EXPORT double SAM_Pvwattsv7_Shading_shading_en_diff_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "shading_en_diff", &result))
		make_access_error("SAM_Pvwattsv7", "shading_en_diff");
	});
	return result;
}
...
SAM_EXPORT double SAM_Pvwattsv7_Shading_shading_en_mxh_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "shading_en_mxh", &result))
		make_access_error("SAM_Pvwattsv7", "shading_en_mxh");
	});
	return result;
}
...
SAM_EXPORT double SAM_Pvwattsv7_Shading_shading_en_string_option_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "shading_en_string_option", &result))
		make_access_error("SAM_Pvwattsv7", "shading_en_string_option");
	});
	return result;
}
...
SAM_EXPORT double SAM_Pvwattsv7_Shading_shading_en_timestep_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "shading_en_timestep", &result))
		make_access_error("SAM_Pvwattsv7", "shading_en_timestep");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Pvwattsv7_Shading_shading_mxh_mget(SAM_table ptr, int* nrows, int* ncols, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_matrix(ptr, "shading_mxh", nrows, ncols);
	if (!result)
		make_access_error("SAM_Pvwattsv7", "shading_mxh");
	});
	return result;
}
...
SAM_EXPORT double SAM_Pvwattsv7_Shading_shading_string_option_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "shading_string_option", &result))
		make_access_error("SAM_Pvwattsv7", "shading_string_option");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Pvwattsv7_Shading_shading_timestep_mget(SAM_table ptr, int* nrows, int* ncols, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_matrix(ptr, "shading_timestep", nrows, ncols);
	if (!result)
		make_access_error("SAM_Pvwattsv7", "shading_timestep");
	});
	return result;
}
...
SAM_EXPORT double SAM_Pvwattsv7_AdjustmentFactors_adjust_constant_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "adjust_constant", &result))
		make_access_error("SAM_Pvwattsv7", "adjust_constant");
	});
	return result;
}
...
SAM_EXPORT double SAM_Pvwattsv7_AdjustmentFactors_adjust_en_hourly_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "adjust_en_hourly", &result))
		make_access_error("SAM_Pvwattsv7", "adjust_en_hourly");
	});
	return result;
}
...
SAM_EXPORT double SAM_Pvwattsv7_AdjustmentFactors_adjust_en_periods_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "adjust_en_periods", &result))
		make_access_error("SAM_Pvwattsv7", "adjust_en_periods");
	});
	return result;
}
...
SAM_EXPORT double SAM_Pvwattsv7_AdjustmentFactors_adjust_en_timeindex_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "adjust_en_timeindex", &result))
		make_access_error("SAM_Pvwattsv7", "adjust_en_timeindex");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Pvwattsv7_AdjustmentFactors_adjust_hourly_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "adjust_hourly", length);
	if (!result)
		make_access_error("SAM_Pvwattsv7", "adjust_hourly");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Pvwattsv7_AdjustmentFactors_adjust_periods_mget(SAM_table ptr, int* nrows, int* ncols, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_matrix(ptr, "adjust_periods", nrows, ncols);
	if (!result)
		make_access_error("SAM_Pvwattsv7", "adjust_periods");
	});
	return result;
}
...
SAM_EXPORT double* SAM_Pvwattsv7_AdjustmentFactors_adjust_timeindex_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "adjust_timeindex", length);
	if (!result)
		make_access_error("SAM_Pvwattsv7", "adjust_timeindex");
	});
	return result;
}
