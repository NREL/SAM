...
SAM_EXPORT int SAM_TcstroughEmpirical_execute(SAM_table data, int verbosity, SAM_error* err){
	return SAM_module_exec("tcstrough_empirical", data, verbosity, err);
}
...
SAM_EXPORT void SAM_TcstroughEmpirical_AdjustmentFactors_adjust_constant_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "adjust_constant", number);
	});
}

SAM_EXPORT void SAM_TcstroughEmpirical_AdjustmentFactors_adjust_en_hourly_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "adjust_en_hourly", number);
	});
}

SAM_EXPORT void SAM_TcstroughEmpirical_AdjustmentFactors_adjust_en_periods_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "adjust_en_periods", number);
	});
}

SAM_EXPORT void SAM_TcstroughEmpirical_AdjustmentFactors_adjust_en_timeindex_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "adjust_en_timeindex", number);
	});
}

SAM_EXPORT void SAM_TcstroughEmpirical_AdjustmentFactors_adjust_hourly_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_array(ptr, "adjust_hourly", arr, length);
	});
}

SAM_EXPORT void SAM_TcstroughEmpirical_AdjustmentFactors_adjust_periods_mset(SAM_table ptr, double* mat, int nrows, int ncols, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_matrix(ptr, "adjust_periods", mat, nrows, ncols);
	});
}

SAM_EXPORT void SAM_TcstroughEmpirical_AdjustmentFactors_adjust_timeindex_aset(SAM_table ptr, double* arr, int length, SAM_error *err){
	translateExceptions(err, [&]{
...
SAM_EXPORT double SAM_TcstroughEmpirical_AdjustmentFactors_adjust_constant_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "adjust_constant", &result))
		make_access_error("SAM_TcstroughEmpirical", "adjust_constant");
	});
	return result;
}
...
SAM_EXPORT double SAM_TcstroughEmpirical_AdjustmentFactors_adjust_en_hourly_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "adjust_en_hourly", &result))
		make_access_error("SAM_TcstroughEmpirical", "adjust_en_hourly");
	});
	return result;
}
...
SAM_EXPORT double SAM_TcstroughEmpirical_AdjustmentFactors_adjust_en_periods_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "adjust_en_periods", &result))
		make_access_error("SAM_TcstroughEmpirical", "adjust_en_periods");
	});
	return result;
}
...
SAM_EXPORT double SAM_TcstroughEmpirical_AdjustmentFactors_adjust_en_timeindex_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "adjust_en_timeindex", &result))
		make_access_error("SAM_TcstroughEmpirical", "adjust_en_timeindex");
	});
	return result;
}
...
SAM_EXPORT double* SAM_TcstroughEmpirical_AdjustmentFactors_adjust_hourly_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "adjust_hourly", length);
	if (!result)
		make_access_error("SAM_TcstroughEmpirical", "adjust_hourly");
	});
	return result;
}
...
SAM_EXPORT double* SAM_TcstroughEmpirical_AdjustmentFactors_adjust_periods_mget(SAM_table ptr, int* nrows, int* ncols, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_matrix(ptr, "adjust_periods", nrows, ncols);
	if (!result)
		make_access_error("SAM_TcstroughEmpirical", "adjust_periods");
	});
	return result;
}
...
SAM_EXPORT double* SAM_TcstroughEmpirical_AdjustmentFactors_adjust_timeindex_aget(SAM_table ptr, int* length, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_array(ptr, "adjust_timeindex", length);
	if (!result)
		make_access_error("SAM_TcstroughEmpirical", "adjust_timeindex");
	});
	return result;
}
...
SAM_EXPORT double* SAM_TcstroughEmpirical_Outputs_annual_energy_distribution_time_mget(SAM_table ptr, int* nrows, int* ncols, SAM_error *err){
	double* result = nullptr;
	translateExceptions(err, [&]{
	result = ssc_data_get_matrix(ptr, "annual_energy_distribution_time", nrows, ncols);
	if (!result)
		make_access_error("SAM_TcstroughEmpirical", "annual_energy_distribution_time");
	});
	return result;
}
