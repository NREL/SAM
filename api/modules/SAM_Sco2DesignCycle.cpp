...
SAM_EXPORT SAM_Sco2DesignCycle SAM_Sco2DesignCycle_construct(const char *def, SAM_error *err) {
    SAM_Sco2DesignCycle result = nullptr;
...
SAM_EXPORT int SAM_Sco2DesignCycle_execute(SAM_Sco2DesignCycle data, int verbosity, SAM_error *err) {
    int n_err = 0;
    translateExceptions(err, [&] {
        n_err += SAM_module_exec("sco2_design_cycle", data, verbosity, err);
    });
    return n_err;
}
...
SAM_EXPORT void
SAM_Sco2DesignCycle_SCO2PowerCycle_I_eta_t_nset(SAM_Sco2DesignCycle ptr, double number, SAM_error *err) {
    translateExceptions(err, [&] {
...
SAM_EXPORT double *
SAM_Sco2DesignCycle_Outputs_O_T_array_des_aget(SAM_Sco2DesignCycle ptr, int *length, SAM_error *err) {
