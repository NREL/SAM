...
SAM_EXPORT void SAM_Singleowner_Common_financial_generation_type_nset(SAM_table ptr, double number, SAM_error *err){
	translateExceptions(err, [&]{
		ssc_data_set_number(ptr, "financial_generation_type", number);
	});
}
...
SAM_EXPORT double SAM_Singleowner_Common_financial_generation_type_nget(SAM_table ptr, SAM_error *err){
	double result;
	translateExceptions(err, [&]{
	if (!ssc_data_get_number(ptr, "financial_generation_type", &result))
		make_access_error("SAM_Singleowner", "financial_generation_type");
	});
	return result;
}
